...
struct buffered_image {
    ...
    const void* data;
    size_t data_size;
    struct msg_owner* owner;  // non-NULL when zero-copy, shared per msg
};
...
static int buffer_image_data(const struct stream2_multidim_array* md,
    ...
    if (compressed) {
        // Store compressed bytes directly; avoid decompressing and copying.
    ...
    if (compressed) {
        ...
        // Zero-copy: keep ownership of the ZMQ frame to retain data lifetime.
        if (*owner_slot == NULL) {
            *owner_slot = calloc(1, sizeof(**owner_slot));
            if (!*owner_slot) {
                return STREAM2_ERROR_OUT_OF_MEMORY;
...
static void free_buffer(struct buffer_ctx* buf) {
    ...
    // Close any remaining owners when refcount reaches zero.
    for (size_t i = 0; i < buf->len; i++) {
        if (buf->items[i].owner && buf->items[i].owner->refs == 0) {
            zmq_msg_close(&buf->items[i].owner->msg);
            free(buf->items[i].owner);
...
int main(int argc, char** argv) {
    ...
    // Raise buffers and high-water mark to avoid dropping bursts.
    int hwm = 10000;
    ...
    for (;;) {
        ...
        // If we used zero-copy, the frame ownership was moved; msg is empty.
    }
